...
애플이 최초에 스위프트를 발표 했을 때, 스위프트 언어적 특성을 safe, Modern, Powerful 이라고 발표했다.
오픈소스로 전환되면서 Safe,Fast,Expressive로 변경하여 다시 발표했지만, 어찌되었든...


## Safe

스위프트는 안정성을 추구한다.
소프트웨어 배포 전에 프로그래머가 저지를수 있는 실수를 엄격한 문법을 적용해서 미연에 방지하고자 노력했다.
때론 너무 강제적이라고 느껴질 수도 있지만 문법적 제재는 실수를 줄이는데 도움이 된다.
버그를 수정하거나 실수를 찾아내는 시간을 절약 할 수 있다.

## Fast

스위프트는 C 언어를 기반으로한 C++, Obj-C 같은 프로그래밍 언어를 대체 하려는 목적으로 만들어 졌다.
아직은 미흡한 부분도 있지만 스위프트는 C언어 수준과 동등한 성능을 일정한 수준으로 유지하는 데 초점을 맞춰서 개발됐다.

## Expressive

컴퓨터 과학 부냥 발전과 함께 성장한 수 많은 프로그래밍 언어 각각의 문법은 다양한 장 단점이 존재한다.
스위프트는 이러한 장단점을 참고해서 좀 더 사용하기 편하고 보기 좋은 문법을 구현하려 노력했다.
덕분에 개발자들이 원하는 현대적이고 세련된 문법을 구사 할수 있다.

위의 세 가지 말고도 많은 특징이 존재한다.
